---
layout: distill
title: Problem 2.3
description: Griffiths Electrodynamics Fourth Edition
img:
importance: 4
category: 
giscus_comments: true
---
Please support the amazing author by purchasing the text. It is a hallmark of physics education and deserves to be on your bookshelf: Griffiths’ Introduction to Electrodynamics 4th Edition [Pearson Education, Inc.]. 

More solutions can be found on my <a href="https://coltonkawamura.github.io/coltonkawamura/Projects/">Projects Page</a>.

Disclaimer: My personal solution may differ from the actual, please use this as a guide for your own solution.

<!----------------------------------------------------------->
<!----------------------Begin Problem------------------------>
<!----------------------------------------------------------->
## Part A

We're dealing with a charge distribution here, instead of a single point charge. That means each little chuck of charge $$dq$$ is going to contribute a little to the overall electric field.

What we want to do is add up all the little contributions to the electric field from each chuck. We can due this by writing Coulomb's law in integral form and integrating from one end $$x=0$$ to the other $$x=L$$,

$$\begin{align*} 
\mathbf{E}=\frac{1}{4\pi\epsilon_{0}} \int_0^L \frac{dq}{r^2}\hat{r}.\tag{1} \label{1}
\end{align*}$$ 

Let's do something about the $$\hat{r}$$ and $$r$$. From inspection of the diagram below, we should expect the electric field to have some $$x$$ and $$z$$ component.

<div class="row">
    <div class="col-sm mt-3 mt-md-0">
        {% include figure.html path="assets/img/griffiths_EM_4E/2-3-1.jpeg" title="example image" class="img-fluid rounded z-depth-1" %}
    </div>
</div>

Thus, we have

$$\begin{align*} 
r=\sqrt{x^2+z^2}
\end{align*}$$ 

and 

$$\begin{align*} 
\hat{r}=z\hat{z}-x\hat{x}.
\end{align*}$$ 


 The value of each $$dq$$ $$q=\lambda / L$$. We're going to need to calculate the electric field from one end of the line segment to the other. When we do this $$r$$ varies. Let's get $$r$$ in a form that includes out given distances. We establish our $$x$$-axis as show below.


From this geometry, we can see that,

$$\begin{align*} 
r=\sqrt{d^2+x^2}.
\end{align*}$$

With this relation, equation \ref{1} becomes 

$$\begin{align*} 
\mathbf{E}=\frac{1}{4\pi\epsilon_{0}}\frac{q}{\left( \sqrt{d^2+x^2} \right)^2}\hat{r}. 
\end{align*}$$


<!----------------------------------------------------------->
<!----------------------End Problem-------------------------->
<!----------------------------------------------------------->
***

If you have any questions or find an error, please leave a comment below and I will reply as soon as I can!

<script src="https://giscus.app/client.js"
        data-repo="coltonkawamura/coltonkawamura"
        data-repo-id="R_kgDOKjP3YQ"
        data-category="comments"
        data-category-id="DIC_kwDOKjP3Yc4CaV1i"
        data-mapping="pathname"
        data-strict="0"
        data-reactions-enabled="1"
        data-emit-metadata="0"
        data-input-position="top"
        data-theme="dark"
        data-lang="en"
        data-loading="lazy"
        crossorigin="anonymous"
        async>
</script>
