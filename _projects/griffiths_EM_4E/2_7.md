...
Let's start with creating an expression for the electric field caused by each $$dq$$. Because the is a spherical shell with $$q$$ smeared out evenly across the surface, we can the infitesimal charge as $$dq=\sigma R^2\sin\theta dr d\phi d \theta$$.

Using this, we can express the electric field it causes as,

$$\begin{align*} 
\mathbf{E}&=\frac{1}{4\pi\epsilon_{0}}\frac{\sigma R^2\sin\theta dr d\phi d \theta}{r^2}\hat{r}. \tag{1} \label{1}
\end{align*}$$
