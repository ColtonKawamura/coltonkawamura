...
## Part C
For the case where $$z>R$$, this is *outside* the sphere, so we expect that that an object with sphereical symmetry to just look like a point charge located at the center of the sphere (as the chapter eluded to). But again, let's check this. When $$z>R$$, $$|z-R|>0$$, and equation \ref{2} becomes,
