...
Let's start by "walking" in the $$x$$ direction first,

$$
\begin{align*}
\int_0^{x'} \mathbf{E}_x \cdot d \mathbf{x} &= \int_0^{x'} (y^2)\hat{x} \cdot dx \hat{x} \\
&= y^2\int_0^{x'} dx \\
&=y^2x.
\end{align*}
$$
...
$$
\begin{align*}
\int_0^{y'} \mathbf{E}_x \cdot d \mathbf{y} &= \int_0^{y'} (y^2)\hat{x} \cdot dx \hat{x} \\
&= y^2\int_0^{x'} dx \\
&=y^2x.
\end{align*}
$$
